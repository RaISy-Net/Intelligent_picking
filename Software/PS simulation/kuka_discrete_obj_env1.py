...
class KukaDiverseObjectEnv(KukaGymEnv):
  ...
  def _randomly_place_objects(self, urdfList):
    ...
    for urdf_name in urdfList:
      xpos = 0.25 * random.random() * self._alpha
      self._alpha*=-1
      ypos = -0.3 - 0.4 * (random.random())
      angle = np.pi / 2 + self._blockRandom * np.pi * random.random()
      orn = p.getQuaternionFromEuler([0, 0, angle])
      urdf_path = os.path.join(self._urdfRoot, urdf_name)
      uid = p.loadURDF(urdf_path, [xpos, ypos, 1.15], [orn[0], orn[1], orn[2], orn[3]])
  ...
  def _step_continuous(self, action):
    ...
    if end_effector_pos[2] <= 0.8:#0.1 for prev gripper
