...
p.setGravity(0, 0, -10)
sphere = p.loadURDF('sphere_small.urdf',[0,0,0])
extra = [];
...
i = 0
'''
logg = p.startStateLogging(p.STATE_LOGGING_VIDEO_MP4,'filename.mp4')
while 1:
	# x=x+0.01
	...
	while(gripper_pos[2]<link2_pos[2]-l2):
		...
		time.sleep(0.01)
	p.resetBasePositionAndOrientation(extra[j],[100,100,100],p.getQuaternionFromEuler([0,0,0]))
	p.resetBasePositionAndOrientation(extra,[100,100,100],p.getQuaternionFromEuler([0,0,0]))
	link2_pos,link2_orn=p.getBasePositionAndOrientation(link2)			
