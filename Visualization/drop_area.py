...
# p.connect(p.GUI)
def MakeArena(x,y,z=0.5,scale_x=1,scale_y=1,scale_z=0.5,Inter_area_dist=1,pickAreaHeight=0.9):
	p.setAdditionalSearchPath(pybullet_data.getDataPath())

	pick_c=p.createCollisionShape(p.GEOM_BOX,halfExtents=[scale_x,scale_y,scale_z+pickAreaHeight/2.0])
	pick_v=p.createVisualShape(p.GEOM_BOX,halfExtents=[scale_x,scale_y,scale_z+pickAreaHeight/2.0],rgbaColor=[1,0,0,1])
	...
	multi=p.createMultiBody(baseCollisionShapeIndex=pick_c,baseVisualShapeIndex=pick_v,basePosition=[x,y_pick,z+pickAreaHeight/2])
